...
void ClientPropertyEditor::renderIdentitySection() {
    ...
    for (size_t i = 0; i < ver_values.size(); ++i)
        if (static_cast<int>(ver_values[i]) == *eb.versionInt) { ver_sel = static_cast<int>(i); break; }
    if (ImGui::BeginCombo("##versioncombo", ver_sel >= 0 ? ver_labels[ver_sel].c_str() : "")) {
        for (size_t i = 0; i < ver_values.size(); ++i) {
            if (ImGui::Selectable(ver_labels[i].c_str(), ver_sel == static_cast<int>(i))) {
                *eb.versionInt = static_cast<int>(ver_values[i]);
                if (cv) {
                    cv->setVersion(static_cast<uint32_t>(*eb.versionInt));
                    std::string dd = std::format("data/{}", *eb.versionInt);
